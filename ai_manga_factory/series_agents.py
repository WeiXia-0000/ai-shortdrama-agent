...
trend_scout_series = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "默认长篇短剧推荐总集数 30-80：preferred_total_episodes_min/max 与 total_episodes 必须自洽（min>=30）。",
            "is_longform_series=true 时：total_episodes 不得小于 preferred_total_episodes_min（不得写‘长篇却偏短’的体量）。",
...
concept_judge_series = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "所有字段必须填写，不得为空。",
            "若某概念 is_longform_series=true 且 total_episodes < preferred_total_episodes_min：必须在该 concept 的 hard_disqualifiers 写入明确条目。",
            "recommended_concept_id 应优先选择 hard_disqualifiers 为空或最少的概念；若无法避免则必须在 fix_suggestions 写明必须修复动作。",
...
anchor_beats_agent = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "must: visible_event/power_shift/payoff_seed/why_audience_remembers_this 四字段必须存在且不为空，用于把 anchor 变成可高密度展开的“可拍承重点”。",
            "must: visible_event 必须写成镜头可见事件（具体到发生了什么、谁在场、可被观众记住）。",
            "must: power_shift 必须明确谁失势/谁获得解释权/资源/安全权，以及转折是怎么发生的。",
            "must: payoff_seed 说明该 anchor 后面要回收到哪里（回收目标应具体，如某次公开处刑、某条规则被验证/推翻、某段身份真相回潮等）。",
            "must: why_audience_remembers_this 必须是高记忆理由（公开翻脸/规则失效/身份将揭未揭/同伴背刺/资源断供/礼法发难等），不能只做文艺总结。",
...
episode_outline_expander_agent = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "must: 每一集必须至少服务于一个：角色成长推进 / 世界揭示推进 / anchor 条件回收 / 公开事件兑现。",
            "must: episode_engine_type 必须明确属于该集的主戏剧引擎（humiliation/reversal/trap/betrayal/alliance_flip/payoff/collapse/reveal 等），不能每集都用同一种“推进型说明集”。",
            "must: episode_goal_in_series 说明这一集在整季中的具体作用，不能只是复述 one_line。",
            "must: anchor_ids 必须能追溯到 anchor_beats 中存在的 anchor_id；每集至少覆盖 1 个，且必须反映“这一集回收哪些承重点”。",
            "must: must_advance 至少 2 条，必须明确本集推进的角色成长/世界揭示/承重点回收中的至少两类。",
            "must: must_payoff 至少 1 条，必须能在本集画面或公开场景中兑现（否则该集密度不足）。",
            "must: must_set_up 至少 1 条，必须为后续某段回收、规则验证或关系爆点提供可用条件。",
            "must: dominant_opposition 必须说清主要压主角的是谁/什么秩序/什么规则/什么关系结构，避免泛写“危机升级”。",
            "must: pressure_arena 必须明确冲突发生的高压场域（家宴/会议室/祠堂/校园围观/副本规则场/资源争夺点/朝会/婚仪/册封/庇护所边界等）。",
            "must: key_turn 必须明确本集最关键的转折是什么，不能是泛话。",
            "must: status_shift/price_paid/relationship_shift/resource_shift 至少三项必须写清“谁上谁下/付出了什么/关系怎么变/资源/权限怎么变”。",
            ...
            "must: 前 3 集不得连续出现「只被压 + 只学规则/只忍 + 无可见回报/无公开站住」式组合；至少穿插可见对抗、筹码、公开站住或资源到手之一。",
...
outline_review_agent = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "must: hard_fail_reasons 还必须覆盖市场治理硬失败逻辑：长篇题材却 total_episodes < 30；前 3 集没有明确可见回报；第 1 集高压但无回报且无 hidden_advantage_seed；front10 桥接过多（bridge 占比>0.5）；front10 留人引擎缺失过多；以及：第 1 集 opening_pressure_level=high/extreme 但 visible_gain_type=none 且 hidden_advantage_seed 为空 且 payoff_deadline 不在 {same_episode,next_episode}；front10 opponent_gain 为空过多（>4）；front10 public_standup_event 全空缺失。",
...
episode_function_agent = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "must: 若输入中当前 episode 合同包含 dominant_opposition/pressure_arena/key_turn/price_paid/relationship_shift/resource_shift 等字段，必须把它们的具体内容映射到本功能卡中（must_advance / what_changes_persistently / what_is_lost / viewer_payoff_design）。",
            ...
            "must: 若输入 current_episode_contract 存在 must_set_up_items（对象数组且含 setup_id），future_threads_strengthened_items 必须逐条覆盖对应 setup_id（允许额外条目）；旧版 future_threads_strengthened 可与各条 description 对齐作为镜像摘要。",
...
episode_script_agent = Agent(
    ...
    instruction=_json_only_instruction(
        ...
        constraints=[
            ...
            "must: narration 不得替对白说出人物本该自己表达的态度、立场与潜台词；不得用旁白复述对白已说内容。",
