...
本版本已升级为更偏制片落地的 **「制作理解入口 + 调度控制台（只读）」**：
- Episode Drawer 增强：展示 `剧情摘要 + 关键转折`，并可展开 `分镜 / 对话 / Seedance 提示词`（来自 storyboard 结构化数据）
- Visual Lock 升级为角色入口：视觉锁角色可点击，打开独立的 Character Drawer
- Character Drawer（角色详情抽屉）：展示角色基础信息、视觉状态/缺项、剧情状态与出场轨迹，并支持点击回跳到 Episode Drawer
- Episode ↔ Character 轻量联动：只在单页内切换抽屉内容，不触发任何写操作/模型调用
...
| Episode Lane（表） | `_iter_episode_dirs`、`gate_artifacts.load_gate_artifact` + `build_gate_trend_summary`、production carry 内按集聚合 |
| Episode Drawer（剧情理解） | `01_episode_function.json`（`episode_goal_in_series` / must_advance / must_inherit / what_changes_persistently / what_is_learned / what_is_mislearned / what_is_lost / future_threads_strengthened`）+ `02_plot.json`（`logic_check` / `cliffhanger`）+ `series_outline.json`（`logline` 等，可缺省降级） |
| Episode Drawer（分镜/对白/Seedance） | `04_storyboard.json`：按 `segments[]` 聚合 scene 位置、narration、dialogue_lines、`seedance_video_prompt`（前端 `<details>` 展开，不改模型流程） |
...
| Visual Lock（角色入口） | `visual_lock_registry.characters`（`complete / partial / missing` + 缺项提示） |
| Character Drawer（角色详情） | `L3_series/02_character_bible.json` / `character_bible.json`（基础与 appearance_lock / consistency_rules）、`L3_series/03_series_memory.json` / `series_memory.json`（角色状态与出场轨迹）、`03b_production_carry_registry.json`（相关 promise/knowledge facts 计数）+ `visual_lock_registry`（视觉缺项/状态） |
